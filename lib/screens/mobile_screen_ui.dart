...
extension _MobileScreenStateUi on _MobileScreenState {
  Widget buildMobileScreen(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_currentTabIndex == 0 ? 'LAN Clip - 发送端' : '触摸板'),
        backgroundColor: _currentTabIndex == 0 
            ? Theme.of(context).colorScheme.inversePrimary 
            : const Color(0xFF0F0F0F),
        foregroundColor: _currentTabIndex == 0 ? null : Colors.white,
        actions: [
      ...
      body: IndexedStack(
        ...
        children: [
          ...
          _selectedDevice != null
              ? EmbeddedTouchpadContent(
                  device: _selectedDevice!,
                  passwordHash: _getPasswordHashForDevice(),
                )
              : const Center(
                  child: Text('请先连接设备', style: TextStyle(color: Colors.grey)),
                ),
        ],
      ),
  ...
  /// 获取当前设备的密码哈希
  String? _getPasswordHashForDevice() {
    if (_selectedDevice != null && _selectedDevice!.requiresPassword) {
      final deviceKey = '${_selectedDevice!.ip}:${_selectedDevice!.port}';
      return _devicePasswords[deviceKey];
    }
    return null;
  }
